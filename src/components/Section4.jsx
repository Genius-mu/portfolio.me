      ...
      <div className="sc4Div w-[80%] sm:w-[65%] py-[4%] flex flex-col items-center gap-y-9">
        ...
        <div className="carousel w-[90%] sm:w-[25em] relative overflow-hidden">
          ...
          <div className="track flex will-change-transform">
            {/** First copy */}
            <div className="group flex items-center gap-x-4 pr-6">
              {skills.map((s, i) => (
                <div
                  key={`a-${i}`}
            ...
            >
              {skills.map((s, i) => (
                <div
                  key={`b-${i}`}
                  className="skill flex-none w-[5.5rem] h-12 rounded-lg flex items-center justify-center bg-red-500 text-white"
                >
                  {s}
                </div>
              ))}
